...
var Component = React.createClass({
  ...
  propTypes: {
    ...
    skipOnClose: React.PropTypes.bool,
   ...
   * Merge the existing set of steps and adds any newly attachable steps
   *
   * @param {Array|Object} steps
   * @param {Array|Object} existingSteps
   * @returns {Array}
   */
  parseSteps: function(steps, existingSteps) {
    var tmpSteps = [],
        newSteps = [],
        el;
    ...
    return this._mergeSteps(existingSteps, newSteps);
  },

  // merge the existing steps and the new steps
  _mergeSteps: function(existingSteps, newSteps) {
    var mergedSteps = [],
        existingIdx = 0,
        newIdx = 0;

    existingSteps = existingSteps ? existingSteps : [];
    while (existingIdx < existingSteps.length && newIdx < newSteps.length) {
      var existingStep = existingSteps[existingIdx];
      var newStep = newSteps[newIdx];
      if (existingStep.stepNo === newStep.stepNo) {
        mergedSteps.push(existingStep);
        existingIdx++;
        newIdx++;
      }
      else if (existingStep.stepNo < newStep.stepNo) {
        mergedSteps.push(existingStep);
        existingIdx++;
      }
      else {
    ...
    if (newIdx < newSteps.length) {
      mergedSteps = mergedSteps.concat(newSteps.slice(newIdx, newSteps.length));
    }
    else if (existingIdx < existingSteps.length) {
      mergedSteps = mergedSteps.concat(existingSteps.slice(existingIdx, existingSteps.length));
    }
    return mergedSteps;
  },
  ...
  _onClickTooltip: function(e) {
    ...
    var skipTypes = props.skipOnClose ? ['skip', 'close'] : ['skip'];
  ...
  _toggleTooltip: function(show, index, action) {
    ...
    this.setState({
      ...
      var lastIndex = action === 'back' ? index + 1 : index - 1,
          lastStep = props.steps[lastIndex];

      if (action && lastStep && lastStep.callbacks) {
        lastStep.callbacks.forEach( function(cb) {
          if (typeof cb === 'function') { cb(lastStep); }
        })
      }

      if (action && typeof props.stepCallback === 'function' && lastStep) {
